-- ========================================
-- DROPDOLLAR COMPLETE PRODUCTION SCHEMA
-- ========================================
-- This is the ONLY schema you need for DropDollar
-- Run this ENTIRE script in your Supabase SQL Editor
-- It includes EVERYTHING: authentication, payments, games, compliance, etc.
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_compliance_log_user_id ON public.location_compliance_log (user_id);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_compliance_log_action ON public.location_compliance_log (action);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_compliance_log_allowed ON public.location_compliance_log (is_allowed);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_compliance_log_created_at ON public.location_compliance_log (created_at DESC);

-- GIN index for location data queries
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_compliance_log_location_data ON public.location_compliance_log USING GIN (location_data);
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_payment_transactions_stripe_id ON public.payment_transactions (stripe_payment_intent_id);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_payment_transactions_status ON public.payment_transactions (status);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_payment_transactions_type ON public.payment_transactions (type);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_payment_transactions_created_at ON public.payment_transactions (created_at DESC);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_payment_transactions_amount ON public.payment_transactions (amount DESC);
...
-- View for monitoring user growth
...
-- View for monitoring payment volume
CREATE OR REPLACE VIEW payment_volume_stats AS
SELECT 
    DATE_TRUNC('day', created_at) as date,
...
-- Function to archive old game sessions (keep database performant)
CREATE OR REPLACE FUNCTION archive_old_game_sessions()
RETURNS void AS $$
BEGIN
    -- Archive game sessions older than 1 year to a separate table
    -- This keeps the main table fast for active queries
...
ALTER TABLE public.payment_transactions SET (fillfactor = 90);
ALTER TABLE public.game_sessions SET (fillfactor = 85);
ALTER TABLE public.user_balances SET (fillfactor = 95);
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_dau_date ON public.daily_active_users (date DESC);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_game_metrics_date ON public.game_performance_metrics (date DESC);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_game_metrics_type ON public.game_performance_metrics (game_type);
...
DO $$
BEGIN
    RAISE NOTICE '🚀 DropDollar Complete Schema Deployed Successfully!';
    RAISE NOTICE '✅ Database is now optimized for millions of users with:';
    RAISE NOTICE '   - Complete authentication system';
    RAISE NOTICE '   - User balances and transactions';
    RAISE NOTICE '   - Game sessions and scoring';
    RAISE NOTICE '   - Location compliance and legal protection';
    RAISE NOTICE '   - Payment processing with Stripe';
    RAISE NOTICE '   - Escrow and seller payouts';
    ...
    RAISE NOTICE '';
    RAISE NOTICE '🎉 Your DropDollar database is production-ready!';
END $$;
