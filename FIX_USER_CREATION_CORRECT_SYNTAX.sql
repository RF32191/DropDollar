-- ============================================================================
-- FIX USER CREATION - CORRECT SYNTAX (ONE ON CONFLICT)
-- Works for all existing and future users
-- ============================================================================

-- STEP 1: Fix Hot Sell join function
...
-- STEP 2: Fix Winner Takes All join function
...
BEGIN
    ...
    IF NOT FOUND THEN
        ...
        IF user_email IS NOT NULL THEN
            generated_username := SPLIT_PART(user_email, '@', 1);
            INSERT INTO public.users (id, username, email, tokens, created_at, updated_at) VALUES (user_id_param, generated_username, user_email, 100, NOW(), NOW()) ON CONFLICT (id) DO UPDATE SET updated_at = NOW();
            SELECT * INTO user_record FROM public.users WHERE id = user_id_param;
        END IF;
    END IF;
...
INSERT INTO public.users (id, username, email, tokens, created_at, updated_at)
SELECT au.id, SPLIT_PART(au.email, '@', 1), au.email, 100, NOW(), NOW()
FROM auth.users au
WHERE au.id NOT IN (SELECT id FROM public.users)
ON CONFLICT (id) DO NOTHING;

-- STEP 5: Fix users with null username
