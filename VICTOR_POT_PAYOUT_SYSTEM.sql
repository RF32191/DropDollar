...
BEGIN
    ...
    -- Add tokens to winner's wallet with explicit confirmation
    UPDATE public.users
    ...
    GET DIAGNOSTICS payout_success = ROW_COUNT;
    
    IF payout_success = 0 THEN
...
-- Test with a different user to ensure it works for all users
INSERT INTO public.winner_takes_all_participants (session_id, user_id, score, accuracy, completed_at, joined_at)
VALUES 
    ((SELECT id FROM public.winner_takes_all_sessions WHERE config_id = 'wta-5-blade-bounce'), 
...
SET 
    ...
    status = 'active',
    updated_at = NOW()
WHERE config_id = 'wta-5-blade-bounce';

-- Show before payout
SELECT 
    'Before Second Victor Payout' as status,
    config_id,
    status,
    current_pot,
    participants_count
...
-- Show winner's tokens before payout
SELECT 
    'Second Winner Tokens Before Payout' as status,
    username,
    email,
    tokens
FROM public.users 
...
-- Execute the victor payout for second user
SELECT public.pay_victor_pot_amount('wta-5-blade-bounce') as second_victor_payout_result;

-- Verify the second winner got paid
SELECT 
    'Second Winner Tokens After Payout' as status,
    username,
    email,
    tokens
FROM public.users 
...
-- Summary of the victor payout system:
...
SELECT 'Victor Pot Payout System Ready!' as final_status;
