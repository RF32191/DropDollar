-- ============================================================================
-- HOT SELL PAYOUT - FORCE ALL TEXT
-- Convert user_id columns to TEXT type to eliminate UUID issues forever
-- ============================================================================

-- Step 1: Ensure user_id in hot_sell_participants is TEXT
DO $$
BEGIN
    -- Check if column is UUID, convert to TEXT
    IF EXISTS (
...
-- Step 2: Drop and recreate the payout function with PURE TEXT
...
BEGIN
    ...
    -- Get 1st place (user_id is now TEXT)
    SELECT 
    ...
    IF v_winner1.user_id_text IS NOT NULL THEN
        ...
        -- Pay 1st (u.id is UUID, p.user_id is TEXT)
        UPDATE public.users 
...
BEGIN
    RAISE NOTICE '========================================';
    RAISE NOTICE '✅ FORCE TEXT PAYOUT CREATED!';
    RAISE NOTICE '========================================';
    RAISE NOTICE '✅ hot_sell_participants.user_id → TEXT';
    RAISE NOTICE '✅ All comparisons use TEXT';
    RAISE NOTICE '✅ JOIN: u.id::text = p.user_id (both TEXT)';
    RAISE NOTICE '✅ WHERE: id::text = user_id (both TEXT)';
    RAISE NOTICE '========================================';
END $$;
