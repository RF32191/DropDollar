-- ============================================================================
-- FIX HOT SELL AUTO RESET AFTER PAYOUT
-- Ensures listings automatically reset and create new sessions after payout
-- ============================================================================

-- STEP 1: Clear all existing sessions to start fresh
DELETE FROM public.hot_sell_participants;
DELETE FROM public.hot_sell_sessions;

-- STEP 2: Recreate fresh sessions
...
-- STEP 3: Fix the payout function to ensure proper reset
...
BEGIN
  ...
  IF v_session_id IS NULL THEN
    RAISE NOTICE '❌ [PAYOUT] No active session found for %', config_id_param;
    RETURN QUERY SELECT FALSE, 'No active session found or already paid out', ''::TEXT, ''::TEXT, ''::TEXT, 0::NUMERIC, 0::NUMERIC, 0::NUMERIC;
    RETURN;
  END IF;

  RAISE NOTICE '✅ [PAYOUT] Found session: %', v_session_id;
  ...
  RAISE NOTICE '📊 [PAYOUT] Participants: %/%', v_participants_count, v_max_participants;

  -- Check if session is full
  IF v_participants_count < v_max_participants THEN
    RAISE NOTICE '⏸️ [PAYOUT] Session not full yet';
  ...
  RAISE NOTICE '✅ [PAYOUT] Completed players: %/%', v_completed_count, v_max_participants;

  IF v_completed_count < v_max_participants THEN
    RAISE NOTICE '⏸️ [PAYOUT] Not all players have completed';
  ...
  RAISE NOTICE '💰 [PAYOUT] Pot: %, Fee: %, Distributable: %', v_current_pot, v_platform_fee, v_distributable_pot;
  RAISE NOTICE '🏆 [PAYOUT] Prizes - 1st: %, 2nd: %, 3rd: %', v_first_prize, v_second_prize, v_third_prize;
  ...
  IF v_first_user_id IS NULL THEN
    RAISE NOTICE '❌ [PAYOUT] No winners found';
    RETURN QUERY SELECT FALSE, 'No winners found', ''::TEXT, ''::TEXT, ''::TEXT, 0::NUMERIC, 0::NUMERIC, 0::NUMERIC;
    RETURN;
  END IF;

  RAISE NOTICE '🥇 [PAYOUT] 1st: % (Score: %, Prize: %)', v_first_username, v_first_score, v_first_prize;
  RAISE NOTICE '🥈 [PAYOUT] 2nd: % (Score: %, Prize: %)', v_second_username, v_second_score, v_second_prize;
  RAISE NOTICE '🥉 [PAYOUT] 3rd: % (Score: %, Prize: %)', v_third_username, v_third_score, v_third_prize;
  ...
  RAISE NOTICE '💸 [PAYOUT] Paid % tokens to %', v_first_prize, v_first_username;
  ...
  IF v_second_user_id IS NOT NULL AND v_second_prize > 0 THEN
    ...
    RAISE NOTICE '💸 [PAYOUT] Paid % tokens to %', v_second_prize, v_second_username;
  END IF;
  ...
  IF v_third_user_id IS NOT NULL AND v_third_prize > 0 THEN
    ...
    RAISE NOTICE '💸 [PAYOUT] Paid % tokens to %', v_third_prize, v_third_username;
  END IF;

  RAISE NOTICE '✅ [PAYOUT] All prizes awarded!';
  ...
  -- CRITICAL: Create a NEW waiting session for this config (auto-reset)
  INSERT INTO public.hot_sell_sessions (
    config_id, 
    current_pot, 
    base_price, 
  ...
  RAISE NOTICE '🔄 [RESET] New session created: % for config: %', v_new_session_id, config_id_param;
  RAISE NOTICE '========================================';
  ...
  RETURN QUERY SELECT 
    TRUE, 
    '🎉 Payout complete! Winners: ' || v_first_username || ' (' || v_first_prize::TEXT || '), ' || 
    COALESCE(v_second_username, 'N/A') || ' (' || COALESCE(v_second_prize, 0)::TEXT || '), ' || 
    COALESCE(v_third_username, 'N/A') || ' (' || COALESCE(v_third_prize, 0)::TEXT || '). New session created!', 
    v_first_username,
...
EXCEPTION
  WHEN OTHERS THEN
    ...
    RETURN QUERY SELECT FALSE, 'Error during payout: ' || SQLERRM, ''::TEXT, ''::TEXT, ''::TEXT, 0::NUMERIC, 0::NUMERIC, 0::NUMERIC;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

GRANT EXECUTE ON FUNCTION public.process_hot_sell_payout(TEXT) TO authenticated, anon;

-- STEP 4: Update get_all_hot_sell_sessions to only show active sessions
...
AS $$
BEGIN
    -- Return only the LATEST session for each config (prioritize waiting/active over completed)
    RETURN QUERY
    ...
    ORDER BY 
        s.config_id,
        CASE WHEN s.status IN ('waiting', 'active') THEN 0 ELSE 1 END, -- Prioritize active sessions
...
BEGIN
    SELECT COUNT(*) INTO session_count FROM public.hot_sell_sessions WHERE status IN ('waiting', 'active');
    ...
    RAISE NOTICE '========================================';
    RAISE NOTICE '✅ HOT SELL AUTO-RESET FIX APPLIED!';
    RAISE NOTICE '========================================';
    RAISE NOTICE '📊 Configs: %', config_count;
    RAISE NOTICE '📊 Active sessions: %', session_count;
    RAISE NOTICE '📊 Current participants: %', participant_count;
    RAISE NOTICE '========================================';
    RAISE NOTICE '✅ Payout will now:';
    RAISE NOTICE '   1. Award prizes to top 3 players';
    RAISE NOTICE '   2. Mark session as completed';
    RAISE NOTICE '   3. Auto-create NEW session';
    RAISE NOTICE '   4. Refresh listings automatically!';
    RAISE NOTICE '========================================';
