...
AND contype = 'c'
AND conname LIKE '%tournament_type%';
...
-- Step 3: Create new check constraint that includes 'winner_takes_all'
...
-- Step 4: Remove the NOT NULL constraint from max_participants
...
-- Step 6: Recreate RLS Policies for winner_takes_all_sessions
...
-- Step 7: Recreate RLS Policies for winner_takes_all_participants
...
-- Step 8: Create Winner Takes It All configurations
...
-- Step 9: Verify the configurations were created
SELECT 
...
-- Step 10: Show all Winner Takes It All configs
SELECT 
    id,
    title,
    game_type,
...
-- Step 12: Test creating a session for the $2 Winner Takes It All
DO $$
DECLARE
    config_id UUID;
    session_id UUID;
