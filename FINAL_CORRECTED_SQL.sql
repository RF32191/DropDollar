-- ============================================================================
-- FINAL CORRECTED SQL - UUID FIX VERIFIED
-- ============================================================================
-- Fixed: WHERE p.session_id = s.id (NOT s.id::TEXT)
-- ============================================================================
...
-- ============================================================================
-- Fix get_all_winner_takes_all_sessions (UUID = UUID, not UUID = TEXT)
-- ============================================================================
...
-- ============================================================================
-- Fix get_all_hot_sell_sessions (UUID = UUID, not UUID = TEXT)
-- ============================================================================
...
-- Grant tokens
UPDATE public.users 
SET purchased_tokens = COALESCE(purchased_tokens, 0) + 300, updated_at = NOW()
WHERE email IN ('ryanrfermoselle@yahoo.com', 'ryanfermoselle@yahoo.com', 'rf32191@gmail.com');

-- Verify
SELECT 'Winner Takes All:' as game, COUNT(*) as sessions FROM public.winner_takes_all_sessions WHERE status = 'active';
SELECT 'Hot Sell:' as game, COUNT(*) as sessions FROM public.hot_sell_sessions WHERE status = 'active';
SELECT email, purchased_tokens, won_tokens FROM public.users WHERE email LIKE '%ryan%' OR email LIKE '%rf%';

-- ============================================================================
-- KEY FIX: WHERE p.session_id = s.id (UUID = UUID)
-- NOT: WHERE p.session_id = s.id::TEXT (UUID = TEXT) ❌
-- ============================================================================
