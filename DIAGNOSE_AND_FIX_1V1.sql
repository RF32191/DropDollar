-- ============================================================================
-- DIAGNOSE AND FIX 1V1 TYPE ISSUE
-- ============================================================================
-- This script will:
-- 1. Show exactly what types exist right now
-- 2. Drop ALL constraints that might block conversion
-- 3. Force convert to TEXT
...
DO $$
DECLARE
    configs_id_type TEXT;
    sessions_config_id_type TEXT;
...
BEGIN
    ...
    -- Check one_v_one_configs.id type
    SELECT data_type INTO configs_id_type
    FROM information_schema.columns
    WHERE table_schema = 'public'
    AND table_name = 'one_v_one_configs'
    AND column_name = 'id';
    
    -- Check one_v_one_sessions.config_id type
    SELECT data_type INTO sessions_config_id_type
    FROM information_schema.columns
    WHERE table_schema = 'public'
    AND table_name = 'one_v_one_sessions'
    AND column_name = 'config_id';
    
    -- Check one_v_one_sessions.id type
    ...
    RAISE NOTICE '📋 one_v_one_configs.id: %', configs_id_type;
    RAISE NOTICE '📋 one_v_one_sessions.config_id: %', sessions_config_id_type;
    RAISE NOTICE '📋 one_v_one_sessions.id: %', sessions_id_type;
    RAISE NOTICE '========================================';
    
    IF configs_id_type = 'uuid' OR sessions_config_id_type = 'uuid' THEN
        RAISE NOTICE '⚠️  TYPE MISMATCH DETECTED!';
        RAISE NOTICE '⚠️  Will convert to TEXT...';
    ELSE
        RAISE NOTICE '✅ Types look correct (TEXT)';
    END IF;
END $$;
...
BEGIN
    ...
    -- Drop FK from one_v_one_sessions to one_v_one_configs
    ALTER TABLE public.one_v_one_sessions
    DROP CONSTRAINT IF EXISTS one_v_one_sessions_config_id_fkey CASCADE;
    
    -- Drop FK from one_v_one_participants to one_v_one_sessions
...
-- ============================================================================
-- STEP 3: FORCE CONVERT one_v_one_configs.id TO TEXT
-- ============================================================================
DO $$
DECLARE
    current_type TEXT;
BEGIN
    ...
    IF current_type = 'uuid' THEN
        ...
        RAISE NOTICE '✅ one_v_one_configs.id → TEXT';
    ELSE
        RAISE NOTICE '✅ one_v_one_configs.id already %', current_type;
    END IF;
END $$;

-- ============================================================================
-- STEP 4: FORCE CONVERT one_v_one_sessions.config_id TO TEXT
-- ============================================================================
DO $$
DECLARE
    current_type TEXT;
BEGIN
    ...
    IF current_type = 'uuid' THEN
        ...
        RAISE NOTICE '✅ one_v_one_sessions.config_id → TEXT';
    ELSE
        RAISE NOTICE '✅ one_v_one_sessions.config_id already %', current_type;
    END IF;
END $$;

-- ============================================================================
-- STEP 5: RECREATE FOREIGN KEY CONSTRAINTS WITH TEXT
-- ============================================================================
DO $$
BEGIN
    RAISE NOTICE '🔗 Recreating foreign key constraints...';
    
    -- FK: one_v_one_sessions.config_id → one_v_one_configs.id
    ALTER TABLE public.one_v_one_sessions
    ...
    -- FK: one_v_one_participants.session_id → one_v_one_sessions.id
    ALTER TABLE public.one_v_one_participants
...
-- ============================================================================
-- STEP 7: ENSURE USER TOKEN COLUMNS
-- ============================================================================
DO $$
BEGIN
    ...
    RAISE NOTICE '✅ User tokens initialized';
END $$;

-- ============================================================================
-- STEP 8: DROP AND RECREATE PAYOUT FUNCTION
-- ============================================================================
...
BEGIN
    RAISE NOTICE '💰 [1V1 PAYOUT] Starting for config: % (type: %)', config_id_param, pg_typeof(config_id_param);
    
    -- Find the active/completed session (config_id is TEXT)
    ...
    RAISE NOTICE '📋 Found session: % (config_id: %)', session_record.id, session_record.config_id;
    ...
    RAISE NOTICE '✅ Session reset triggered';
...
-- ============================================================================
-- STEP 9: DROP AND RECREATE RESET FUNCTION
-- ============================================================================
...
DECLARE
    configs_id_type TEXT;
    sessions_config_id_type TEXT;
    payout_function_exists BOOLEAN;
    reset_function_exists BOOLEAN;
BEGIN
    SELECT data_type INTO configs_id_type
    FROM information_schema.columns
    WHERE table_schema = 'public'
    ...
    SELECT EXISTS(
        ...
        WHERE proname = 'process_1v1_payout'
    ) INTO payout_function_exists;
    
    SELECT EXISTS(
        SELECT 1 FROM pg_proc 
        WHERE proname = 'reset_1v1_session'
    ) INTO reset_function_exists;
    
    RAISE NOTICE '========================================';
    RAISE NOTICE '✅ 1V1 SYSTEM DIAGNOSIS COMPLETE!';
    RAISE NOTICE '========================================';
    RAISE NOTICE '✅ one_v_one_configs.id: %', configs_id_type;
    RAISE NOTICE '✅ one_v_one_sessions.config_id: %', sessions_config_id_type;
    RAISE NOTICE '✅ process_1v1_payout() exists: %', payout_function_exists;
    RAISE NOTICE '✅ reset_1v1_session() exists: %', reset_function_exists;
    
    IF configs_id_type = 'text' AND sessions_config_id_type = 'text' THEN
        RAISE NOTICE '========================================';
        RAISE NOTICE '🎉 SUCCESS! Both columns are TEXT';
        RAISE NOTICE '🎉 No more "text = uuid" errors!';
        RAISE NOTICE '🎉 Fair skill-based gaming enabled';
        RAISE NOTICE '🎉 Scalable for millions of players';
        RAISE NOTICE '========================================';
    ELSE
        RAISE NOTICE '========================================';
        RAISE NOTICE '⚠️  WARNING: Type mismatch still exists!';
