-- ============================================================================
-- COMPLETE USERNAME AND MESSAGE FIX - V2 (Fixed Schema)
-- ============================================================================
...
-- ============================================================================
-- STEP 0: Add username column to participants table if not exists
-- ============================================================================
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE table_name = 'marketplace_participants' 
        AND column_name = 'username'
    ) THEN
        ALTER TABLE public.marketplace_participants 
        ADD COLUMN username TEXT;
        
        RAISE NOTICE '✅ Added username column to marketplace_participants';
    ELSE
        RAISE NOTICE 'ℹ️ username column already exists';
    END IF;
END $$;

SELECT '✅ Step 0: Schema updated' as status;

-- ============================================================================
-- STEP 1: Populate usernames for all participants
-- ============================================================================
...
-- ============================================================================
-- STEP 2: Create/Replace update_marketplace_score to set username
-- ============================================================================

DROP FUNCTION IF EXISTS public.update_marketplace_score(TEXT, UUID, NUMERIC, NUMERIC) CASCADE;
DROP FUNCTION IF EXISTS public.update_marketplace_score(UUID, UUID, NUMERIC, NUMERIC) CASCADE;
...
DECLARE
    ...
    v_username TEXT;
BEGIN
    RAISE NOTICE '🎮 Updating score: session=%, user=%, score=%', session_id_param, user_id_param, score_param;
    
    -- Get username
    SELECT COALESCE(
        (SELECT username FROM public.users WHERE id = user_id_param),
        (SELECT split_part(email, '@', 1) FROM public.users WHERE id = user_id_param),
        'Player'
    ) INTO v_username;
    
    -- Update participant with score, accuracy, username, and completed_at
    ...
    RAISE NOTICE '✅ Score updated: username=%, score=%, completed_at=%', 
        v_username, score_param, v_participant.completed_at;
...
GRANT EXECUTE ON FUNCTION public.update_marketplace_score(UUID, UUID, NUMERIC, NUMERIC) TO authenticated;

SELECT '✅ Step 2: update_marketplace_score sets username' as status;

-- ============================================================================
-- STEP 3: Fix process_marketplace_winner to SET winner_username
-- ============================================================================
...
SELECT '✅ Step 3: process_marketplace_winner sets winner_username' as status;

-- ============================================================================
-- STEP 4: Rebuild auto-message trigger
-- ============================================================================
...
SELECT '✅ Step 4: Auto-message trigger created' as status;

-- ============================================================================
-- STEP 5: Process ALL existing completed sessions and send messages
-- ============================================================================
...
SELECT '✅ Step 5: All messages sent to existing winners and sellers' as status;

-- ============================================================================
-- STEP 6: Update get_all_marketplace_listings
-- ============================================================================
...
SELECT '✅ Step 6: Username lookup function updated' as status;
...
SELECT '
╔════════════════════════════════════════════════════════════════╗
║   ✅ COMPLETE USERNAME & MESSAGE FIX V2 APPLIED                ║
╚════════════════════════════════════════════════════════════════╝

WHAT WAS FIXED:

0️⃣ SCHEMA FIXED:
   ✅ Added username column to marketplace_participants
   ✅ Safe migration (IF NOT EXISTS)

1️⃣ REAL USERNAMES:
   ✅ Stored in participants table
   ✅ Updated on score submission
   ✅ Falls back to account username → email → "Player"

2️⃣ WINNER DISPLAY:
   ✅ winner_username set by process_marketplace_winner
   ✅ Shows in gold banner on listing
   ✅ Visible to everyone

3️⃣ AUTO-MESSAGES:
   ✅ Trigger fires on session completion
   ✅ Sends to winner AND seller
   ✅ Checks for duplicates
   ✅ All existing sessions backfilled
...
SELECT 
    ...
    COUNT(*) FILTER (WHERE winner_username IS NOT NULL AND winner_username != '') as with_username
