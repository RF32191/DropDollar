-- ============================================================================
-- FIX LEVEL CALCULATION - ENSURE USER AT 171 XP SHOWS LEVEL 2 CORRECTLY
-- ============================================================================
-- The issue: User has 171 XP but progress bar shows 0 XP
-- Formula: Level N requires 100 * N^1.5 XP
-- Level 1: 100 XP (0-100)
-- Level 2: 283 XP (100-383 total)
-- User with 171 XP should be Level 2 with 71 XP earned in Level 2
-- ============================================================================

-- ============================================================================
-- 1. FIX award_xp TO CORRECTLY CALCULATE LEVELS
-- ============================================================================
...
BEGIN
    ...
    SELECT ux.total_xp, ux.current_level
    ...
    WHILE v_cumulative_xp <= v_new_xp LOOP
...
-- ============================================================================
-- 2. FIX get_user_xp TO RECALCULATE LEVEL FROM TOTAL_XP
-- ============================================================================
...
BEGIN
    ...
    WHILE v_cumulative_xp <= v_total_xp LOOP
...
-- ============================================================================
-- 3. VERIFY LEVEL CALCULATION FOR USER WITH 171 XP
-- ============================================================================

DO $$
DECLARE
    v_user_id UUID;
    v_total_xp INTEGER := 171;
    v_calculated_level INTEGER;
    v_cumulative_xp INTEGER := 0;
    v_temp_level INTEGER;
...
BEGIN
    ...
    -- Calculate level from 171 XP
    v_calculated_level := 1;
    v_cumulative_xp := 0;
    
    WHILE v_cumulative_xp <= v_total_xp LOOP
        v_xp_for_level := FLOOR(100 * POWER(v_calculated_level, 1.5))::INTEGER;
        v_cumulative_xp := v_cumulative_xp + v_xp_for_level;
        
        RAISE NOTICE 'Level %: % XP needed, cumulative: %', v_calculated_level, v_xp_for_level, v_cumulative_xp;
    ...
    v_cumulative_xp := 0;
    v_temp_level := 1;
    WHILE v_temp_level < v_calculated_level LOOP
        v_cumulative_xp := v_cumulative_xp + FLOOR(100 * POWER(v_temp_level, 1.5))::INTEGER;
        v_temp_level := v_temp_level + 1;
    END LOOP;
    ...
    v_xp_earned_in_level := v_total_xp - v_cumulative_xp;
    v_xp_for_level := FLOOR(100 * POWER(v_calculated_level, 1.5))::INTEGER;
    v_xp_to_next := v_xp_for_level - v_xp_earned_in_level;
    
    RAISE NOTICE '';
    RAISE NOTICE '========================================';
    RAISE NOTICE 'LEVEL CALCULATION VERIFICATION';
    RAISE NOTICE '========================================';
    RAISE NOTICE 'Total XP: %', v_total_xp;
    RAISE NOTICE 'Calculated Level: %', v_calculated_level;
    RAISE NOTICE 'Cumulative XP (previous levels): %', v_cumulative_xp;
    RAISE NOTICE 'XP earned in Level %: %', v_calculated_level, v_xp_earned_in_level;
    RAISE NOTICE 'XP needed for Level %: %', v_calculated_level, v_xp_for_level;
    RAISE NOTICE 'XP to next level: %', v_xp_to_next;
    RAISE NOTICE 'Progress: % percent', ROUND((v_xp_earned_in_level::NUMERIC / v_xp_for_level::NUMERIC) * 100, 2);
    RAISE NOTICE '';
    ...
    RAISE NOTICE '✅ Updated user level to %', v_calculated_level;
...
SELECT '✅ Level calculation fixed! User with 171 XP should now show Level 2 correctly.' as status;
