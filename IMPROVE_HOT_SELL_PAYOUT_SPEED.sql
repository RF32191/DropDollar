-- ============================================================================
-- IMPROVE HOT SELL PAYOUT SPEED
-- Add immediate payout trigger and save winners to analytics
-- ============================================================================

-- Update the payout function to save to analytics
...
BEGIN
  ...
  SELECT s.id, s.current_pot, s.max_participants, c.game_type
  INTO v_session_id, v_current_pot, v_max_participants, v_game_type
  ...
  -- Save to game_history for analytics (1st place)
  INSERT INTO game_history (user_id, game_type, score, tokens_won, created_at)
  VALUES (v_first_user_id, v_game_type, v_first_score, v_first_prize, NOW());

  -- Save 2nd place
  IF v_second_user_id IS NOT NULL THEN
    INSERT INTO game_history (user_id, game_type, score, tokens_won, created_at)
    VALUES (v_second_user_id, v_game_type, v_second_score, v_second_prize, NOW());
  END IF;

  -- Save 3rd place
  IF v_third_user_id IS NOT NULL THEN
    INSERT INTO game_history (user_id, game_type, score, tokens_won, created_at)
    VALUES (v_third_user_id, v_game_type, v_third_score, v_third_prize, NOW());
  END IF;

  RAISE NOTICE '✅ Session marked completed and saved to analytics';
...
BEGIN
    RAISE NOTICE '========================================';
    RAISE NOTICE '✅ Hot Sell payout improved!';
    RAISE NOTICE '✅ Winners now saved to game_history';
    RAISE NOTICE '✅ Analytics will show winners';
    RAISE NOTICE '✅ Immediate session reset';
    RAISE NOTICE '========================================';
END $$;
