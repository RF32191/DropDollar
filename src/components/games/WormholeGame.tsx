...
export default function WormholeGame({ onGameEnd, isCompetitive = false }: WormholeGameProps) {
  ...
  useEffect(() => {
    ...
    const gameLoop = () => {
      ...
      // Check room transition (crossing bridge from Room 1 to Room 2)
      if (!hasTriggeredTransitionRef.current && currentRoom === 1) {
        // Bridge is at x=15-30, y>20
        if (player.position.x > 25 && player.position.y > 18) {
          // Player is crossing to Room 2!
  ...
  return (
    <div className="fixed inset-0 bg-black">
      ...
      {showRoomTransition && (
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/90 backdrop-blur-sm transition-opacity">
          <div className="text-center">
            <div className="text-6xl mb-6 animate-pulse">🌀</div>
            <h2 className="text-3xl font-bold text-white mb-4">
              {transitionProgress < 50 ? 'Entering Wormhole...' : 'Welcome to Chamber 2'}
            </h2>
            ...
            <div className="w-64 h-3 bg-gray-700 rounded-full overflow-hidden mx-auto">
              <div 
                className="h-full bg-gradient-to-r from-green-500 via-cyan-500 to-purple-500 transition-all duration-100"
                style={{ width: `${transitionProgress}%` }}
              />
            </div>
            <div className="text-gray-500 text-sm mt-2">{transitionProgress}%</div>
          </div>
        </div>
      )}
        ...
        <>
          ...
          <div className="absolute top-4 left-4 z-40">
            <div className="bg-black/70 rounded-lg px-4 py-2 space-y-1">
              ...
              <div className={`text-sm font-bold ${currentRoom === 1 ? 'text-blue-400' : 'text-purple-400'}`}>
                🏠 Chamber {currentRoom}
              </div>
            </div>
          </div>
