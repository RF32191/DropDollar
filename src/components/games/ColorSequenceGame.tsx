...
export default function ColorSequenceGame({ onGameEnd, onExit, listingId, entryNumber, isCompetitionMode, gameId }: ColorSequenceGameProps) {
  ...
  if (gameState === 'ready') {
    return (
      <div 
        className="fixed inset-0 bg-gradient-to-br from-red-900 via-purple-900 to-black bg-opacity-95 flex items-center justify-center z-50 backdrop-blur-sm overflow-y-auto cursor-pointer"
        onClick={handleStartGame}
      >
        <div className="absolute top-8 left-0 right-0 z-50 pointer-events-none">
          <div className="text-center bg-gradient-to-r from-purple-400 to-pink-500 text-white text-3xl font-black py-4 px-8 rounded-full inline-block animate-pulse shadow-2xl">
