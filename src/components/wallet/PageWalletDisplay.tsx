...
type WalletVariant = 'default' | 'hot-sell' | 'winner-takes-all' | '1v1';
...
export default function PageWalletDisplay({ variant = 'default' }: PageWalletDisplayProps) {
  ...
  return (
    <div className="mb-6 grid grid-cols-1 md:grid-cols-3 gap-4">
      {/* Total Balance */}
      <div className={`bg-gradient-to-br ${colors.total.bg} backdrop-blur-xl rounded-2xl p-6 border ${colors.total.border} shadow-lg hover:scale-105 transition-all duration-300 relative overflow-hidden group animate-pulse-slow`}>
        {/* Animated border glow */}
        <div className={`absolute inset-0 rounded-2xl bg-gradient-to-r ${colors.total.bg} opacity-0 group-hover:opacity-100 transition-opacity duration-300 blur-xl`}></div>
        
        <div className="relative z-10">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              <BanknotesIcon className={`w-6 h-6 ${colors.total.icon} animate-bounce`} />
              <span className={`text-sm font-semibold ${colors.total.text} uppercase tracking-wide`}>Total Balance</span>
            </div>
          </div>
          <div className={`text-4xl font-bold ${colors.total.value} mb-1`}>{totalTokens.toFixed(2)}</div>
          <div className={`text-sm ${colors.total.text}`}>Tokens Available</div>
        </div>
      </div>

      {/* Purchased Tokens Wallet */}
      <div className={`bg-gradient-to-br ${colors.purchased.bg} backdrop-blur-xl rounded-2xl p-6 border ${colors.purchased.border} shadow-lg hover:scale-105 transition-all duration-300 relative overflow-hidden group`}>
        {/* Animated border glow */}
        <div className={`absolute inset-0 rounded-2xl bg-gradient-to-r ${colors.purchased.bg} opacity-0 group-hover:opacity-100 transition-opacity duration-300 blur-xl`}></div>
        
        <div className="relative z-10">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              <ShoppingCartIcon className={`w-6 h-6 ${colors.purchased.icon} animate-pulse`} />
              <span className={`text-sm font-semibold ${colors.purchased.text} uppercase tracking-wide`}>Purchased Tokens</span>
            </div>
          </div>
          <div className={`text-4xl font-bold ${colors.purchased.value} mb-1`}>{purchasedTokens.toFixed(2)}</div>
      ...
      <div className={`bg-gradient-to-br ${colors.winnings.bg} backdrop-blur-xl rounded-2xl p-6 border-2 ${colors.winnings.border} shadow-xl hover:shadow-2xl transition-all duration-300 relative overflow-hidden group`}>
        {/* Glow effect */}
        <div className={`absolute inset-0 bg-gradient-to-br ${colors.winnings.glow || 'from-green-400/10 to-emerald-400/10'} opacity-0 group-hover:opacity-100 transition-opacity duration-300`}></div>
        ...
        <div className="relative z-10">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              <TrophyIcon className={`w-6 h-6 ${colors.winnings.icon} animate-pulse`} />
              <span className={`text-sm font-semibold ${colors.winnings.text} uppercase tracking-wide`}>💰 Winnings Wallet</span>
            </div>
          </div>
          <div className={`text-4xl font-bold ${colors.winnings.value} mb-2`}>{wonTokens.toFixed(2)}</div>
