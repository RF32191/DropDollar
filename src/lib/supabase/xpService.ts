...
export class XPService {
  ...
  static calculateXPProgress(xpData: UserXPData): number {
    ...
    // CRITICAL: Calculate progress based on XP earned in current level
    ...
    // Return calculated progress (0-100), clamped
    return Math.max(0, Math.min(100, calculatedProgress));
  }
  
  // Fallback method (kept for compatibility but not used)
  static calculateXPProgressOld(xpData: UserXPData): number {
