-- ========================================
-- COMPLETE AUTONOMOUS DROPDOLLAR PRODUCTION SCHEMA - FIXED
...
-- ========================================
-- STEP 15: CREATE PERFORMANCE INDEXES (FIXED)
-- ========================================
...
-- Function to update high scores and personal bests (SIMPLIFIED)
...
BEGIN
    -- Check if this is a personal best (simplified without COALESCE issues)
    ...
    WHERE user_id = NEW.user_id 
      ...
      AND (tournament_id = NEW.tournament_id OR (tournament_id IS NULL AND NEW.tournament_id IS NULL))
      AND (hot_sell_id = NEW.hot_sell_id OR (hot_sell_id IS NULL AND NEW.hot_sell_id IS NULL));
    ...
    IF existing_score IS NULL OR NEW.score > existing_score THEN
        ...
        ON CONFLICT (user_id, game_type, listing_id, tournament_id, hot_sell_id)
        DO UPDATE SET
            score = NEW.score,
            session_id = NEW.id,
...
-- ========================================
-- STEP 19: FINAL SCHEMA REFRESH & OPTIMIZATION
-- ========================================

-- Create simple composite indexes (without WHERE clauses that caused issues)
CREATE INDEX idx_listings_active_playable ON public.listings (status, is_playable, created_at DESC);
CREATE INDEX idx_tournaments_open_1v1 ON public.tournaments (status, is_1v1_match, starts_at);
CREATE INDEX idx_hot_sell_active_prize ON public.hot_sell_competitions (status, prize_amount, starts_at);
CREATE INDEX idx_game_sessions_leaderboard ON public.game_sessions (game_type, score DESC, completed_at DESC);
CREATE INDEX idx_high_scores_global_records ON public.high_scores (game_type, score DESC, created_at DESC);
CREATE INDEX idx_matchmaking_active_queue ON public.matchmaking_queue (game_type, skill_rating, entry_fee, created_at);

-- Create partial indexes for performance (simplified)
CREATE INDEX idx_users_active_players ON public.users (total_games_played DESC, created_at DESC);
CREATE INDEX idx_user_skill_ratings_active ON public.user_skill_ratings (game_type, current_rating DESC, games_played DESC);
