-- ============================================================================
-- COMPLETE USER BACKUP SYSTEM
-- Ensures ALL user data, transactions, and history are saved to Supabase
-- ============================================================================

-- ============================================
-- 1. USERS TABLE (Already exists - verify)
-- ============================================
-- Stores complete user profiles and balances
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'users' AND column_name = 'tokens') THEN
        ALTER TABLE public.users ADD COLUMN tokens NUMERIC DEFAULT 10;
    END IF;
...
-- ============================================
-- Permanent record of EVERY token movement
...
CREATE TABLE IF NOT EXISTS public.token_transactions (
    ...
    competition_type TEXT, -- 'hot_sell', 'winner_takes_all', '1v1', 'listing'
...
-- ============================================
-- 4. GAME HISTORY TABLE (Enhanced)
-- ============================================
-- Every game played by every user - PERMANENT
...
CREATE INDEX IF NOT EXISTS idx_game_history_user_date ON public.game_history(user_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_game_history_type ON public.game_history(game_type);
CREATE INDEX IF NOT EXISTS idx_game_history_competition ON public.game_history(competition_type);

-- ============================================
-- 5. USER GAME HISTORY TABLE (Dashboard)
-- ============================================
-- Specifically for user dashboard display
...
CREATE INDEX IF NOT EXISTS idx_user_game_history_user ON public.user_game_history(user_id, created_at DESC);

-- ============================================
-- 6. USER ACTIVITY LOG
-- ============================================
-- Track all user actions for analytics and support
...
CREATE TABLE IF NOT EXISTS public.user_activity_log (
    ...
    -- Activity Details
    action_type TEXT NOT NULL, -- 'login', 'logout', 'purchase', 'game_played', 'token_transfer'
    description TEXT,
    ip_address TEXT,
    user_agent TEXT,
...
CREATE POLICY "Users can view own dashboard history" ON public.user_game_history FOR SELECT USING (auth.uid() = user_id);
...
CREATE POLICY "Service can insert game history" ON public.game_history FOR INSERT WITH CHECK (true);
CREATE POLICY "Service can insert dashboard history" ON public.user_game_history FOR INSERT WITH CHECK (true);
CREATE POLICY "Service can insert activity" ON public.user_activity_log FOR INSERT WITH CHECK (true);

-- ============================================
-- 8. AUTOMATIC BACKUP TRIGGERS
-- ============================================
...
BEGIN
    IF NEW.type IN ('purchase', 'bonus') THEN
        ...
        SET 
            ...
            updated_at = NOW()
        WHERE id = NEW.user_id;
    ELSIF NEW.type IN ('game_entry', 'spend') THEN
        UPDATE public.users
        SET 
...
GRANT SELECT, INSERT ON public.user_game_history TO authenticated, anon;
...
BEGIN
    ...
    AND table_name IN ('token_transactions', 'purchase_history', 'game_history', 'user_game_history', 'user_activity_log');
    ...
    RAISE NOTICE '========================================';
    RAISE NOTICE '📊 Backup tables created: %', table_count;
    ...
    RAISE NOTICE '   ✓ Every game played';
    RAISE NOTICE '   ✓ Dashboard game history';
