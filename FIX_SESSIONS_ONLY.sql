-- ============================================================================
-- FIX SESSION ISSUES (MAINTAINS ANTI-CHEAT SECURITY)
-- ============================================================================
-- ✅ Creates missing sessions
-- ✅ Fixes join functions with UUID handling
-- ✅ Maintains ALL anti-cheat security measures
-- ============================================================================
...
SELECT 'Winner Takes All:' as game, COUNT(*) as active_sessions FROM public.winner_takes_all_sessions WHERE status = 'active';
SELECT 'Hot Sell:' as game, COUNT(*) as active_sessions FROM public.hot_sell_sessions WHERE status = 'active';

SELECT '=== JOIN FUNCTIONS ===' as info;
SELECT proname as function_name, COUNT(*) as versions 
FROM pg_proc 
WHERE proname IN ('join_hot_sell_session', 'join_winner_takes_all_session')
GROUP BY proname;

SELECT '=== ANTI-CHEAT SECURITY ===' as info;
SELECT 'spend_tokens:' as component, CASE WHEN EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'spend_tokens') THEN '✅ ACTIVE' ELSE '❌ MISSING' END as status
UNION ALL
SELECT 'game_sessions:', CASE WHEN EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'game_sessions') THEN '✅ ACTIVE' ELSE '❌ MISSING' END
UNION ALL
SELECT 'anti_cheat_logs:', CASE WHEN EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'anti_cheat_logs') THEN '✅ ACTIVE' ELSE '❌ MISSING' END;

-- ============================================================================
-- COMPLETE: Sessions fixed, skill-based gaming security maintained! 🛡️
-- ============================================================================
