-- ============================================================================
-- ULTIMATE FIX FOR CHALLENGES AND RP WALLET - SCALABLE FOR MILLIONS OF USERS
-- ============================================================================
-- This ensures:
-- 1. Daily/weekly tasks update correctly for ALL users
-- 2. System scales to millions of users
-- 3. RP wallet maintains and gets payouts correctly
-- 4. Optimized with proper indexes
-- ============================================================================

-- ============================================================================
-- 1. CREATE INDEXES FOR PERFORMANCE (CRITICAL FOR SCALE)
-- ============================================================================
...
CREATE INDEX IF NOT EXISTS idx_daily_challenges_date_type ON public.daily_challenges(challenge_date, challenge_type, is_active);
CREATE INDEX IF NOT EXISTS idx_user_daily_challenges_user_challenge ON public.user_daily_challenges(user_id, challenge_id);
CREATE INDEX IF NOT EXISTS idx_user_daily_challenges_updated ON public.user_daily_challenges(updated_at DESC);

-- Indexes for weekly challenges
CREATE INDEX IF NOT EXISTS idx_weekly_challenges_week_type ON public.weekly_challenges(week_start_date, challenge_type, is_active);
...
CREATE INDEX IF NOT EXISTS idx_user_weekly_challenges_updated ON public.user_weekly_challenges(updated_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_rp_transactions_user_created ON public.reward_points_transactions(user_id, created_at DESC);

-- ============================================================================
-- 2. ENSURE TRIGGER FIRES CORRECTLY AND IS OPTIMIZED
-- ============================================================================
...
BEGIN
    ...
    IF TG_OP = 'INSERT' THEN
        ...
        -- Use PERFORM to execute without waiting for return value (faster)
...
-- ============================================================================
-- 3. OPTIMIZED update_challenges_on_game_complete (FAST FOR MILLIONS)
-- ============================================================================
...
-- ============================================================================
-- 4. OPTIMIZED update_daily_challenge_progress (FAST, SCALABLE)
-- ============================================================================
...
-- ============================================================================
-- 5. OPTIMIZED update_weekly_challenge_progress (FAST, SCALABLE)
-- ============================================================================
...
-- ============================================================================
-- 6. ENSURE RP WALLET FUNCTIONS ARE OPTIMIZED
-- ============================================================================

-- Verify get_user_xp is fast (should use index on user_id)
-- This function is already optimized, but ensure it exists
...
-- ============================================================================
-- 7. VERIFY ALL FUNCTIONS EXIST AND ARE CORRECT
-- ============================================================================
...
BEGIN
    ...
    FROM (
        SELECT 'award_practice_game_xp' as proname
        UNION SELECT 'award_competition_game_xp'
        UNION SELECT 'generate_daily_challenges'
        UNION SELECT 'generate_weekly_challenges'
    ) required
    WHERE NOT EXISTS (
...
BEGIN
    ...
    RAISE NOTICE '   - Functions optimized for scale';
    RAISE NOTICE '   - Trigger optimized for speed';
    RAISE NOTICE '';
    RAISE NOTICE '💰 RP WALLET:';
    RAISE NOTICE '   - RP awarded automatically on challenge completion';
    RAISE NOTICE '   - RP balance maintained in user_xp.reward_points';
    RAISE NOTICE '   - Transactions recorded for audit';
    RAISE NOTICE '';
    RAISE NOTICE '📊 CHALLENGES:';
    RAISE NOTICE '   - Auto-generated if missing';
    RAISE NOTICE '   - Progress updates immediately';
    RAISE NOTICE '   - Works for millions of users';
    RAISE NOTICE '';
END $$;

SELECT '✅ Ultimate fix applied! System optimized for scale and RP wallet maintained.' as status;
