...
BEGIN
  ...
  IF v_auth_user_id IS NULL THEN
    RAISE NOTICE '⚠️  No auth user found for rf32191@gmail.com - user needs to register first';
  ELSE
    ...
    IF v_existing_profile IS NOT NULL THEN
      ...
      -- Update to ensure tokens are correct
      UPDATE public.users
      SET 
        purchased_tokens = COALESCE(purchased_tokens, 0) + 300,
        total_tokens = COALESCE(total_tokens, 0) + 300,
        updated_at = NOW()
      WHERE id = v_auth_user_id
      AND purchased_tokens < 300;
      
      IF FOUND THEN
        RAISE NOTICE '✅ Added 300 tokens to existing profile';
      END IF;
    ELSE
      -- Create the profile
...
BEGIN
    ...
    RAISE NOTICE '   • User profile handled for rf32191@gmail.com';
    RAISE NOTICE '';
