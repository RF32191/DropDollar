-- FIX_SHOW_COMPLETED_LISTINGS.sql
-- This fixes the marketplace listings to:
-- 1. Show completed sessions (not just active)
-- 2. Read timer from listing table (not hardcoded)
-- 3. Include all winner/claim fields
...
BEGIN
    ...
    SELECT 
        ...
        COALESCE(ml.timer_duration, 180)::INTEGER as timer_duration, -- 🎯 READ FROM LISTING, default 3 min
        ...
        COALESCE(ms.rng_seed, ml.rng_seed, floor(random() * 99999 + 1)::integer)::INTEGER as rng_seed,
    ...
    WHERE 
        ml.status != 'deleted' -- ✅ SHOW ACTIVE, WAITING, AND COMPLETED!
...
-- Verify the fix
SELECT '✅ Fixed get_all_marketplace_listings to show completed sessions with correct timer!' as result;

-- Show current listings by status
SELECT 
