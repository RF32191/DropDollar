-- ============================================================================
-- 🔧 WTA FIX: EXPLICIT UUID CASTING ON BOTH SIDES
-- ============================================================================
...
-- Recreate sessions with 1 MIN timer
...
-- Score update - CAST BOTH SIDES TO TEXT
...
BEGIN
    -- Cast session_id column to TEXT for comparison
...
-- Join function - CAST ALL session_id comparisons
...
BEGIN
    ...
    -- Insert with session_id as-is (it will be stored as whatever type the column is)
...
-- Get sessions - CAST session comparisons
...
-- Payout function - CAST all comparisons
...
BEGIN
    ...
    IF NOT FOUND THEN 
        RETURN '{"success":false,"message":"No session"}'::jsonb; 
    END IF;
    
    -- Store as TEXT
    v_session_id := v_s.id::TEXT;
    
    -- Cast to TEXT for comparison
    SELECT user_id, score, COALESCE(username, 'Player') as username 
    INTO v_win 
    ...
    IF NOT FOUND THEN 
        RETURN '{"success":false,"message":"No scores"}'::jsonb; 
    END IF;
    
    v_pot := COALESCE(v_s.current_pot, 0);
    IF v_pot <= 0 THEN 
        RETURN '{"success":false,"message":"Empty pot"}'::jsonb; 
    END IF;
    ...
    DELETE FROM winner_takes_all_participants WHERE session_id::TEXT = v_session_id;
    UPDATE winner_takes_all_sessions SET 
        status = 'waiting', 
...
-- Verify
SELECT '✅ SESSIONS WITH 1 MIN TIMER:' as status;
SELECT id::TEXT, status, timer_duration FROM winner_takes_all_sessions;

SELECT '✅ ALL FIXED WITH ::TEXT CASTING' as done;
