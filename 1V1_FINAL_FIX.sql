-- ============================================================================
-- 1V1 FINAL FIX - WORKS WITH ANY COLUMN TYPE
-- ============================================================================
-- This handles both TEXT and UUID config_id columns
-- ============================================================================
...
-- ============================================================================
-- PAYOUT FUNCTION - Uses dynamic SQL to handle both TEXT and UUID
-- ============================================================================
...
DECLARE
    ...
    v_config_id_for_query TEXT;
BEGIN
    v_config_id_for_query := config_id_param;
    
    RAISE NOTICE '💰 [1V1 PAYOUT] Starting for config: %', config_id_param;
    
    -- Find active session - this works for both TEXT and UUID columns
    SELECT * INTO session_record
    FROM public.one_v_one_sessions
    WHERE config_id::TEXT = v_config_id_for_query
    ...
    IF NOT EXISTS (
        ...
        BEGIN
            EXECUTE format(
                'INSERT INTO public.one_v_one_sessions (id, config_id, status, participants_count, current_pot, rng_seed, created_at, updated_at) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)'
            ) USING gen_random_uuid(), session_record.config_id, 'waiting', 0, 0, session_record.rng_seed, NOW(), NOW();
...
BEGIN
  RAISE NOTICE '';
  RAISE NOTICE '✅ ========================================';
  RAISE NOTICE '✅ 1V1 FUNCTIONS UPDATED - FINAL VERSION!';
  RAISE NOTICE '✅ ========================================';
  RAISE NOTICE '✅ join_1v1_session - READY';
  RAISE NOTICE '✅ process_1v1_payout - READY';
  RAISE NOTICE '✅ Works with TEXT or UUID config_id';
  RAISE NOTICE '✅ Uses config_id::TEXT for comparison';
